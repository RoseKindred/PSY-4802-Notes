...
```{r}
diamonds %>% group_by(color) %>% summarize(meanPrice = mean(price), sd(price))
```
...
data(penguins)
flipperBySpecies<-summarySE(penguins, measurevar = "flipper_length_mm", groupvars = "species", na.rm = TRUE)
flipperBySpecies
```
...
```{r}
ggplot(diamonds, aes(price))+geom_histogram(color = "purple", fill = "pink")+theme_classic()
```
...
```{r}
ggplot(penguins, aes(bill_length_mm))+geom_histogram(bins = 40)
```
2. Include a theme to change the appearance of your graph.
```{r}
ggplot(penguins, aes(bill_length_mm))+geom_histogram(bins = 40)+theme_bw()
```

3. Create a histogram with orange bars and purple lines. 
```{r}
ggplot(penguins, aes(bill_length_mm))+geom_histogram(bins= 40, color = "purple", fill = "orange")+theme_bw()
```

4. **Bonus** Create a histogram with the worst color combination you can think of. 
```{r}
ggplot(penguins, aes(bill_length_mm))+geom_histogram(bins = 40, color = "yellow", fill = "olivedrab1")+theme(panel.background = element_rect(fill = "coral2"))
```
...
```{r}
ggplot(diamonds, aes(clarity))+geom_bar(color = "black", fill = "lightgreen")+theme_bw()
```
...
```{r}
ggplot(diamonds, aes(clarity))+geom_bar(color = "mediumorchid4", fill = "lightsteelblue") +theme_bw()+ xlab("Clarity of Diamond")+ylab("Frequency")+ggtitle("Graph of Diamond Clarity")
```

you can also choose to use labs, with x, y, and title
```{r}
ggplot(diamonds, aes(clarity))+geom_bar(color = "lavender", fill = "lightsteelblue") +theme_classic()+ labs(title = "Graph of Diamond", x = "Diamond Clarity", y = "Frequency")
```

Note - \n- starts a new line 
```{r}
ggplot(diamonds, aes(clarity))+geom_bar(color = "lavender", fill = "lightsteelblue") +theme_bw()+ labs(title = "Graph of Diamond", x = "Diamond \nClarity", y = "Frequency")
```
...
```{r}
ggplot(priceByColor, aes(color, price))+geom_bar(stat = "identity", fill= "lightblue")+geom_errorbar(aes(ymin = price-se, ymax = price+se))+theme_bw()
```

```{r}
ggplot(priceByColor, aes(color, price))+geom_bar(stat = "identity", fill= "lightblue", alpha = .8)+geom_errorbar(aes(ymin = price-se, ymax = price+se, width = .5))+theme_bw()
```
...
```{r}
ggplot(priceByColor, aes(color, price))+geom_bar(stat = "identity", fill= "lightblue", alpha = .8)+geom_errorbar(aes(ymin = price-se, ymax = price+se, width = .5))+theme_bw()+geom_hline(yintercept = mean(diamonds$price), size =1, linetype = "dashed", color = "steelblue")

```

```{r}
ggplot(priceByColor, aes(color, price))+geom_bar(stat = "identity", fill= "lightblue", alpha = .8)+geom_errorbar(aes(ymin = price-se, ymax = price+se, width = .5))+theme_bw()+geom_vline(xintercept = "G", color = "purple")

ggplot(priceByColor, aes(color, price))+geom_bar(stat = "identity", fill= "lightblue", alpha = .8)+geom_errorbar(aes(ymin = price-se, ymax = price+se, width = .5))+theme_bw()+geom_vline(xintercept = 2, color = "darkgreen")
```
...
```{r}
data(penguins)
ggplot(penguins, aes(species))+geom_bar(color = "black", fill= "white", alpha = .8)+theme_bw()
```

```{r}
ggplot(penguins, aes(species))+geom_bar(color = "black", fill= "white", alpha = .8)+theme_bw()+xlab("Species")+ylab("Frequency")+ggtitle("Penguins")
```
### Adding a second variable to a bar graph:
```{r}
ggplot(diamonds, aes(color, fill = cut))+geom_bar()+theme_bw()
ggplot(diamonds, aes(color, fill = cut))+geom_bar(position = position_dodge())+theme_bw() #categorical
```
```{r}
ggplot(diamonds, aes(color, fill = price))+geom_bar()+theme_bw()
```
```{r}
data(mtcars)
ggplot(mtcars, aes(cyl, fill = as.factor(mpg)))+geom_bar(position = position_dodge())+theme_bw()
```
...
```{r}
ggplot(diamonds, aes(carat, price))+geom_point(color = "steelblue")+theme_bw()
```

```{r}
ggplot(diamonds, aes(carat, price))+geom_point(color = "steelblue", size = 2, shape = 7)+theme_bw()
```

#jitter()
```{r}
ggplot(mtcars, aes(cyl, gear))+geom_point(color = "steelblue", size = 2, shape = 12)+theme_bw()
ggplot(mtcars, aes(cyl, gear))+geom_jitter(color = "steelblue", size = 2, shape = 12)+theme_bw()
```

### Practice
- make a scatter plot of bill length and bill depth. 
```{r}
ggplot(penguins, aes(bill_length_mm, bill_depth_mm))+geom_point(color = "darkgreen", size = 2, shape = 6)+theme_bw()
```

- modify the graph to include at least 2of the style elements we talked about
```{r}
ggplot(penguins, aes(bill_length_mm, bill_depth_mm))+geom_point(color = "red", size = 9, shape = 40)+theme_bw()
```
...
```{r}
ggplot(diamonds, aes(carat, price))+geom_point(color = "steelblue", size = 2, shape = 7)+theme_bw()+geom_smooth(method = "lm", size = 1, color = "purple", linetype = "dashed")
```
...
```{r}
ggplot(mtcars, aes(mpg, hp))+geom_point(color = "steelblue", size = 2, shape = 7)+theme_bw()+geom_smooth(method = "lm")
ggplot(mtcars, aes(mpg, hp))+geom_point(color = "steelblue", size = 2, shape = 7)+theme_bw()+geom_smooth(method = "lm", se = FALSE)
```
**Changing scatterplots**
you can change the outline of a dot using "color" 
```{r}
ggplot(diamonds, aes(carat, price, color = color))+geom_point()+theme_bw()
```

####Practice: 
1. Create a scatter plot of the body mass and flipper length variables. 
```{r}
ggplot(penguins, aes(body_mass_g, flipper_length_mm))+geom_point()+theme_bw()
```
2. Change the color of the points to your favorite color. 
```{r}
ggplot(penguins, aes(body_mass_g, flipper_length_mm))+geom_point(color = "steelblue")+theme_bw()
```
3. Change the shape of the points. 
```{r}
ggplot(penguins, aes(body_mass_g, flipper_length_mm))+geom_point(color = "steelblue", size = 5, shape = 23)+theme_bw()
```
4. Add a line of best fit to your scatter plot. 
```{r}
ggplot(penguins, aes(body_mass_g, flipper_length_mm))+geom_point(color = "steelblue", size = 5, shape = 23)+theme_bw()+geom_smooth(method = "lm", color = "black")
```
...
```{r}
ggplot(diamonds, aes(color, price))+ geom_boxplot(color = "black", fill = "lightblue")
```

```{r}
ggplot(diamonds, aes(color, price))+ geom_boxplot(color = "black")+geom_jitter()
ggplot(mtcars, aes(as.factor(cyl), mpg))+ geom_boxplot(color = "steelblue")+geom_jitter()
```

##Violin Plot
```{r}
ggplot(mtcars, aes(as.factor(cyl), mpg))+geom_violin()+geom_jitter()
```
...
```{r best graph}
custom_colors <- c("0" = "lightblue2","1" = "steelblue","2" = "steelblue1","3" = "steelblue2", "4" = "steelblue3", "5" = "steelblue4")
ggplot(StudentMHDat, aes(cortisolLevel, fill = as.factor(stressLevel)))+geom_bar()+theme_bw()+scale_fill_manual(values = custom_colors)+xlab("Cortisol Levels")+ylab("Stress Level Frequency")+ggtitle("Student Cortisol and Stress Levels")+guides(fill = guide_legend(title = "Rating of Stress"))
```


```{r worst graph}
ggplot(StudentMHDat, aes(stressLevel, cortisolLevel))+geom_point(color = "maroon1", size = 6, shape = 40)+theme(panel.background = element_rect(fill = "olivedrab1"))+geom_smooth(method = "lm", color = "orange")
```
...
use fill = 
```{r}
ggplot(diamonds, aes(price, fill = color))+geom_histogram(bins = 15)
```
...
```{r}
ggplot(diamonds, aes(price, fill = color))+geom_histogram(position = position_dodge(), bins = 10)+scale_fill_manual(values = c("maroon1", "maroon2", "maroon3", "maroon", "maroon4", "darkorchid4", "purple4"))
```
`scale_color_manual(values = #)`; 
```{r}
ggplot(diamonds, aes(price, carat, color = cut))+geom_point()+scale_color_manual(values = c("maroon1", "maroon2", "maroon3", "maroon", "maroon4", "darkorchid4", "purple4"))
```

R ColorBrewer
`scale_color_brewer(palette=)`
```{r}
ggplot(diamonds, aes(price, carat, color = cut))+geom_point()+scale_color_brewer(palette = "PiYG")

ggplot(diamonds, aes(price, fill = color))+geom_histogram()+scale_fill_brewer(palette = "Set2")
```
...
```{r}
library(viridis)
ggplot(diamonds, aes(price, fill = color))+ geom_histogram()+scale_fill_viridis(discrete = TRUE, option = "F")

ggplot(diamonds, aes(price, carat, color = cut))+ geom_point()+scale_color_viridis(discrete = TRUE, option = "B")
```
...
```{r}
ggplot(penguins, aes(bill_length_mm, fill = island))+geom_histogram(position = position_dodge(), bins = 10)+scale_fill_viridis(discrete = TRUE, option = "B")+theme_bw()+xlab("Bill Length")+ylab("Frequency")+ggtitle("Frequency of Bill Length on Each Island")+guides(fill = guide_legend(title = "Island"))
```

2. In some way, include species as a third variable in this graph. 
```{r}
ggplot(penguins, aes(bill_length_mm, island, fill = species))+geom_boxplot()+theme_bw()+xlab("Bill Length")+ylab("Island")+ggtitle("Frequency of Bill Length on Each Island")+guides(fill = guide_legend(title = "Species"))
```

3. Create a graph of bill length and bill depth
```{r}
ggplot(penguins, aes(bill_length_mm, fill = island))+geom_histogram(position = position_dodge(), bins = 10)+scale_fill_brewer(palette = "Set3")+theme_bw()+xlab("Bill Length")+ylab("Frequency")+ggtitle("Frequency of Bill Length on Each Island")+guides(fill = guide_legend(title = "Island"))
```
