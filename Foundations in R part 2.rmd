...
```{r}
"is.classname"
is.factor(mtcars$automan)

"as.classname"
#make a new column 
mtcars$automan<- as.factor(mtcars$automan)
mtcars$automan

mtcars$automan<- factor(mtcars$automan)
class(mtcars$automan)
#going from factor to numeric can cause issues and there's a specific way to do it
```
**DOUBLE CHECK YOUR DATA**
```{r}
mtcars$automan2<-as.numeric(mtcars$automan)
mtcars$automan2

mtcars %>% select(automan, automan2) #compare before saving

mtcars$automan3<-as.numeric(levels(mtcars$automan)[mtcars$automan])
mtcars %>% select(automan, automan3)
```
...
```{r}
BySeven<-function(number){number*7}
```
test it out
```{r}
BySeven(8)
...
`paste()`
```{r}
myFavortie<-function(thing){paste("my favorite food is", thing)}
```
test
```{r}
myFavortie("pasta")
...
```{r}
divideByHundred<-function(number){number/100}
```

2. Test your function on the mtcars mpg column.
```{r}
divideByHundred(mtcars$mpg)
```

3. Make a function that finds the mean of a column and divides it by the standard deviation of another column.
```{r}
columnMeanAndSD<-function(number1, number2){mean(number1)/sd(number2)}
```

4. Test it on the mpg column and hp column. 
```{r}
columnMeanAndSD(mtcars$mpg, mtcars$hp)
```

```{r}
MeanSQ<-function(number1, number2){mean(sqrt(number1))+mean(number2)}
MeanSQ(mtcars$disp, mtcars$qsec)
```
...
```{r}
VectorFor<-cbind.data.frame(10:100)
VectorFor$NewColumn<-NA
for (i in 1:nrow(VectorFor)) {VectorFor$NewColumn[i]<-VectorFor$`10:100`[i]*4}
VectorNum<-10:100
for(i in 1:length(VectorNum)){VectorNum[i]*4}
for (placeholder in 1:length(VectorNum)) {print(VectorNum[placeholder]*4)
...
```{r}
GoodBadMPG<-(mtcars$NewColumn<-NA)
for (i in 1:nrow(mtcars)) {mtcars$GoodBadMPG[i]<-mtcars$mpg[i]>25}
```
...
```{r}
mtcars$above25<- NA
for (i in 1:nrow(mtcars)) {if(mtcars$mpg[i]>25){mtcars$above25[i]<-"Good Mileage"}}
```
### else statement
```{r}
for (i in 1:nrow(mtcars)) {if(mtcars$mpg[i]>25){mtcars$above25[i]<-"Good Mileage"}else{mtcars$above25[i]<-"Bad Mileage"}}
```
...
```{r}
ChickWeight$above100<- NA
for (i in 1:nrow(ChickWeight)) {if(ChickWeight$weight[i]>100){ChickWeight$above100[i]<-"Big Chick"} else{ChickWeight$above100[i]<-"Small Chick"}}
```
for(){
  if(){
...
```{r}
data(penguins)
penguins$favIsland<- NA
for (i in 1:nrow(penguins)) {
  if(penguins$island[i]=="Torgersen"){penguins$favIsland[i]<-"Hyde"
  } 
  if(penguins$island[i]=="Dream"){penguins$favIsland[i]<-"Rose"
  } 
  if(penguins$island[i]=="Biscoe"){penguins$favIsland[i]<-"Jekyll"}}
```
