...
```{r}
AggressionDat <- read.csv("~/PSY 4802 Notes/PSY4802-AggressionSampleData.csv")
...
**Wide data:** There is only one participant per row. Each column represents a unique measurement. (in our example, the data is currently in wide format).
...
```{r}
religIncomeLong<- pivot_longer(relig_income, cols = c(2:10), names_to = "IncomeAmt", values_to = "NumberParticipants")
```
...
```{r}
library(reshape2)
melt(AggressionDat, id.vars= c("subjNo", "ageGroup", "parentReportGender", "condition"), measure.vars = c("baselineAggression", "postAggression"), variable.names = "timepoint", value.name = "aggressionScore")
```
...
```{r}
billboardLong<- pivot_longer(billboard, cols = c(4:79), names_to = "WeeksBillboard", values_to = "ChartRating")
```
...
```{r}
aggressionWide<- pivot_wider(aggressionLongDat, id_cols = c("subjNo", "ageGroup", "condition", "parentReportGender"), names_from = "timePoint", values_from = "aggressionScore")
```
...
```{r}
longDat2<- spread(aggressionLongDat, key = "timePoint", value = "aggressionScore")
```
...
```{r}
wideChick2<- pivot_wider(ChickWeight, id_cols = c("Chick", "Diet"), names_from = "Time", values_from = "weight")
```

3. Convert from wide to long. 
```{r}
longChick2<-pivot_longer(wideChick2, cols = c(3:14), names_to = "Week", values_to = "WeightofChickie")
```
...
```{r}
wideSleep<- pivot_wider(sleep, id_cols = c("ID"), names_from = "group", values_from = "extra")
```

4) Go back to the chick weight dataset and take all data from chick 3 and put it in wide format 
```{r}
Chick3<-subset()
wideChick3<-pivot_wider(ChickWeight, id_cols = c("Chick", "Diet"), names_from = "Time", values_from = "weight")
```
...
anxietyAggressionDat<-left_join(AggressionDat, anxietyDat, by= "subjNo")
```

`right_join()` does the same thing, but places the new column to the right. 
```{r}
aggressionAnxietyDat<-right_join(anxietyDat, AggressionDat)
```
...
```{r}
simulatedExecFuncScore<-sample(size=48, x=0:16, replace = TRUE)

execFuncDat<- data.frame(subjNo = AggressionDat$subjNo, simulatedExecFuncScore)
```

2. Join your dataset in #1 to the original Aggression dataset. 
```{r}
execAgresssionDat<- left_join(execFuncDat, AggressionDat)
```
...
is.factor(AggressionDat$parentReportGender2)
```
...
levels(AggressionDat$parentReportGender2)
```
...
```{r}
AggressionDat %>% select(parentReportGender, parentReportGender2)
```
...
```{r}
levels(AggressionDat$condition2)<-c("e", "i")
```

3. Check that you have done this successfully. 
```{r}
AggressionDat %>% select(condition, condition2)
```
...
```{r}
AggressionDat$condition2<- relevel(AggressionDat$condition2, ref = "i")
AggressionDat$condition2
```
...
#### Practice
1. Reorder the condition variable in your aggression dataset so that inclusion is the reference category. 
```{r}
AggressionDat$condition2<- relevel(AggressionDat$condition2, ref = "i")
AggressionDat$condition2
```
...
```{r}
recode(testOutcome, "'strongly Agree' = 4; 'agree' = 3; 'disagree' = 2; 'strongly Disagree' = 1")
```
...
```{r}
data("starwars")
starwars %>% mutate(meters = height/100)
```
