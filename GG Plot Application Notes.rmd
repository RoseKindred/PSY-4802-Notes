---
title: "GG Plot Application Notes"
...
```{r}
set.seed(137)
stateUniversity <- sample(c("UMN", "UMD", "UMich", "KU", "UGA", "NYU", "Rutgers", "URI", "UPenn", "OSU", "PSU", "UDel", "UConn", "UCLA", "UC Riverside", "UCSB", "UCM", "NDSU", "USC", "UNC"), 100,  replace = TRUE)
...
```{r}
stdErr<- function(column){sd(column)/sqrt(length(column))}
summaryBoba<- bobaDat %>% group_by(schoolClimate, studentInterest) %>% dplyr::summarise(Mean = mean(boba), stdev = sd(boba), stderr = stdErr(boba))
```
```{r}
...
```{r}
ggplot(summaryBoba, aes(schoolClimate, Mean, fill = studentInterest))+geom_bar(stat="identity", position = position_dodge(), width = 1)+geom_errorbar(aes(ymin = Mean - stderr, ymax = Mean+stderr), position = position_dodge2(), width = 1)
```
...
using `facet wrap()`
```{r}
ggplot(bobaDat, aes(studentSize, boba))+geom_point()+geom_smooth(method = "lm")+theme_bw()+facet_wrap(~studentInterest)
```

using color 
```{r}
ggplot(bobaDat, aes(studentSize, boba, color = studentInterest))+geom_point()+geom_smooth(method = "lm")+theme_bw()+facet_wrap(~studentInterest)
```
...
using ggeffects package
```{r}
lm3<-lm(boba~ studentSize*radius, data = bobaDat)
a3<- ggpredict(lm3)
plot(a3)
```
...
server <- function(input, output) {
  output$hist <- renderPlot({
    ggplot(, aes(rnorm(input$number)))+ geom_histogram(fill="darksalmon")+theme_bw()
  })
}
...
ui <- fluidPage(
  sliderInput(inputId = "number", 
    label = "Pick a number", 
    value = 2500, min = 0, max = 37500), plotOutput("hist")
)

server <- function(input, output) {
  output$hist <- renderPlot({
    ggplot(, aes(rnorm(input$number)))+ geom_histogram(fill="steelblue3")+theme_bw()
  })
}
...
ui <- fluidPage(
  sliderInput(inputId = "number", 
    label = "Pick a number", 
    value = 2500, min = 0, max = 5000), plotOutput("boxp")
)
