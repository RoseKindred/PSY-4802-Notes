...
```{r}
library(readr)
smh<- read_csv("students_mental_health_survey fall 2025 (1).csv")
```
...
range(smh$stressLevel)[2] - range(smh$stressLevel)[1]
```
...
#### Pipes
```{r}
smh %>% summarize(stDev = sd(stressLevel), variance = var(stressLevel), interQuart = IQR(stressLevel), range = range(stressLevel)[2] - range(stressLevel)[1])
```
...
```{r}
smh$stressLevelZ <- scale(smh$stressLevel)
smh$centeredStress <- scale(smh$stressLevel, center = TRUE, scale = FALSE)
...
They are very similar to one another, which should mean that it should be a normal distribution, but that is not necessarily true. 
...
```{r}
ggplot(smh, aes(cortisolLevel))+geom_histogram()+ geom_vline(xintercept = mean(smh$cortisolLevel), color = "orchid", size = 1)+theme_bw()+geom_vline(xintercept = median(smh$cortisolLevel), size = 1, linetype = "dashed", color = "purple4")+ geom_vline(xintercept = mfv(smh$cortisolLevel), size = 1, color = "steelblue")
```
...
**chi squared tests** 
compare frequencies of nominal data. Goodness of fit test: do our data differ from expected proportions?
test of independence: are our two variable related?
